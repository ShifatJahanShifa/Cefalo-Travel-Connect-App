...
class Notifcation implements INotification {
    ...
    async getNotificationByUserId(user_id: string): Promise<notification[]> {
        const result: notification[] = await db("notifcations").select("*").where({user_id: user_id}).orderBy("created_at", "desc")
        return result
    }

    async getNotificationByNotificationId(notification_id: string): Promise<notification> {
        const result: notification = await db("notifcations").select("*").where({notification_id: notification_id}).first()
        return result
    } 

    async markAsRead(notification_id: string): Promise<notification> {
        const [result] = await db("notifcations").update({
                        read: true
    ...
    async deleteNotiffication(notification_id: string): Promise<boolean> {
        await db("notifcations").where({notification_id: notification_id}).del()
        return true
    }
}
